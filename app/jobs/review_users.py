    ...
    with GetDB() as db:
        ...
        for user in get_users_for_review(db, now_ts):
            ...
            try:
                ...
                db.rollback()
                logger.warning(f"User {user.username} was modified or deleted during review, skipping.")
        ...
        for user in get_onhold_users_for_review(db, now):
            try:
                ...
                db.rollback()
                logger.warning(f"User {user.username} was modified or deleted during on-hold review, skipping.")
