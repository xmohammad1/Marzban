...
class NextPlanModel(BaseModel):
    ...
    def validate_expire(cls, v):
        ...
        if v > MAX_UNIX_TIMESTAMP:
            raise ValueError(f"expire cannot exceed {MAX_UNIX_TIMESTAMP}")
        return v
...
class User(BaseModel):
    ...
    def validate_on_hold_expire_duration(cls, v):
        ...
        if now_ts + int(v) > MAX_UNIX_TIMESTAMP:
            raise ValueError(
                "on_hold_expire_duration results in expire beyond supported range"
            )
        return v
